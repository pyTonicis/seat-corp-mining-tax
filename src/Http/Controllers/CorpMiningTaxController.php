...
class CorpMiningTaxController extends Controller
{
    ...
    public function getData2($cid = 0)
    {
        ...
        $html .= "<tbody><tr>";
        $html .= "<td><b>Contract to</b></td><td id='c_name'>" .$details->character_name ."<button class='btn' onclick=\"copy('#c_name')\" data-copy='#c_name' data-done='copied'><i class='fas fa-copy'></i></button></td></tr><tr>";
        $html .= "<td><b>Contract Title</b></td><td id='c_title'>" .$details->contractTitle ."<button class='btn' onclick=\"copy('#c_title')\" data-copy='#c_title' data-done='copied'><i class='fas fa-copy'></i></button></td></tr><tr>";
        $html .= "<td><b>Contract Type</b></td><td>ItemExchange</td></tr><tr>";
        $html .= "<td><b>Tax</b></td><td id='c_tax'>" .number_format($details->tax). "<button class='btn' onclick=\"copy('#c_tax')\" data-copy='#c_tax' data-done='copied'><i class='fas fa-copy'></i></button></td></tr></tbody></table>";
        $html .= "<script>function copy(id) { document.getElementById(id).select(); document.execCommand('copy'); }</script>";
        $response['html'] = $html;
